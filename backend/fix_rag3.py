...
in_prompt = False
for line in lines:
    if 'def _build_direct_answer_prompt(self)' in line:
        in_prompt = True
        new_lines.append(r'''    def _build_direct_answer_prompt(self) -> PromptTemplate:
...
''')
    ...
    if in_prompt:
        if 'return PromptTemplate.from_template(prompt_text)' in line:
            in_prompt = False
        continue
